...
class login_window:
    def __init__(self, root):
        ...
        self.bg2 = ImageTk.PhotoImage(file = "image/admin_login.jpg")
        bg2 = Label(self.root , image = self.bg2).place(x=550, y=150 , width=550 ,height=400)

        self.bg3 = ImageTk.PhotoImage(file="image/4faebb2031bb2787fda144179e3662a7.jpg")
        bg3 = Label(login_frame, image=self.bg3).place(x=0, y=0, width=350, height=80)

        label = Label(login_frame, text="Welcome to Student Management System",font=('times new roman',12,'bold'),justify=CENTER,bg="white",fg='blue')
        ...
        username= Label(login_frame,text="Username",font=('Bell MT',16,'bold'),justify=CENTER,bg="white",fg='black').place(x=50,y=130)
        self.txt_email = Entry(login_frame, font=("times new roman", 15), bg="light grey")
        self.txt_email.place(x=20, y=170, width=280)
        ...
        password = Label(login_frame, text="Password", font=('Bell MT', 16, 'bold'), justify=CENTER, bg="white",fg='black').place(x=50, y=205)
        ...
        btn_reg=Button(login_frame,text="Register new Account?",command=self.Register_window,font=("Times new roman",13,'bold'),bg="white",bd=0,fg="green",cursor="hand2")
        btn_reg.place(x=10,y=285)

        btn_forget=Button(login_frame,text="Forget Password?",command=self.forget_password,font=("Times new roman",13,'bold'),bg="white",bd=0,fg="red",cursor="hand2")
        btn_forget.place(x=190,y=285)

        self.btn_image1 = ImageTk.PhotoImage(file="image/download1.jpg")
        btn_login = Button(login_frame,image=self.btn_image1,command=self.login_data,bd=0, bg="CRIMSON", fg="white",cursor="hand2")
        btn_login.place(x=60, y=320,width=200,height=50)

        # ========footer ==============#
        footer = Label(self.root, text="Developed By : Amit Anand Chawarekar           Email-ID : amit.chawarekar@gmail.com                           Contact no. :  9503016634,9921663430",
    ...
    def login_data(self):
       if self.txt_email.get() == "" or self.txt_password.get() == "":
           ...
           try:
               ...
               cur=con.cursor()
               cur.execute("select * from admin_table where email=%s and password=%s",(self.txt_email.get(),self.txt_password.get()))
               row=cur.fetchone()
    ...
    def reset_password(self):
        if self.combo_question.get()=="Select" or self.txt_answer.get()== "" or self.txt_new_paswword.get()=="":
            ...
            try:
                con = pymysql.connect(host='localhost', user='root', password="", database="srms1")
                cur = con.cursor()
                cur.execute("select * from admin_table where email=%s and question=%s and answer=%s",
                ...
                if row == None:
                    ...
                    cur.execute("update admin_table set password=%s where email=%s",
    ...
    def forget_password(self):
        if self.txt_email.get() == "":
            ...
            try:
                con = pymysql.connect(host='localhost', user='root', password="", database="srms1")
                cur = con.cursor()
                cur.execute("select * from admin_table where email=%s",
                            (self.txt_email.get()))
                row = cur.fetchone()
