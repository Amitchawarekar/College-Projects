...
from Manage_employee import ManageEmp
...
class SMS:
    def __init__(self,root):
        ...
        btn_mangage_emp = Button(self.root, text="Manage Employee", font=("goudy old style", 15, "bold"), bg="brown", fg="white",
                            cursor="hand2", command=self.manage_emp).place(x=50, y=10, width=190, height=32)
        #=====Content Window ================
    ...
    def manage_emp(self):
        self.new_win = Toplevel(self.root)
        self.new_obj = ManageEmp(self.new_win)

    def export(self):

        connection = pymysql.connect(host="localhost", user="root", password="", database="srms1")
